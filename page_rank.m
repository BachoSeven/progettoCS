...
for i = 1:length(gamma)
    b = ((1-gamma(i))/n) * e;
    M = eye(n) - gamma(i) * A * D^(-1);
    tic;
    [x, relres, it, resvec] = gmres_arnoldi(M, b, tol);
    elapsed = toc;
    resvecs{i} = resvec;
    disp(['Numero di iterazioni per gamma = ', num2str(gamma(i)), ': ', num2str(it)]);
    disp(['Tempo impiegato per gamma = ', num2str(gamma(i)), ': ', num2str(elapsed), ' secondi']);
end
